df_summarise <- readRDS("processed_outputs/df_summarise_scenario6.rds") %>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC")))
df_summarise_totals <- readRDS("processed_outputs/df_summarise_totals_scenario6.rds")%>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC")))
...
ggsave("plots/scenario6_impact_time.png", height = 8, width = 10)
...
ggsave("plots/scenario6_summary.png", height = 4.5, width = 8)
