...
scenarios <- read_csv("scenarios_counter_lmic.csv", show_col_types = FALSE)
df <- list.files(path = "raw_outputs/output_counter_lmic/", pattern = ".rds")
df <- map(paste0("raw_outputs/output_counter_lmic/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/df_summarise_counter_lmic.rds")
...
ggsave("plots/counterfactual_lmic.png", height = 8, width = 8)
