df_summarise <- readRDS("processed_outputs/df_summarise_scenario5_sensitivity.rds") %>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC"))) %>%
  filter(!(strategy_name != "Counterfactual" & timestep < 300))

df_summarise_totals <- readRDS("processed_outputs/df_summarise_totals_scenario5_sensitivity.rds") %>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC")))
...
p0 <- ggplot(data = filter(df_summarise, max_coverage == 0.8), aes(x = as.Date(date), y  = vaccines_t/target_pop, col = strategy_name)) +
  geom_line(size = 1) +
...
combined <- p0 / p1 / (p2 | p3) + plot_annotation(tag_levels = "A")+ plot_layout(guides = "collect") + plot_layout(heights = c(1,2,2))
combined
ggsave("plots/scenario5_summary.png", height = 8, width = 10)
