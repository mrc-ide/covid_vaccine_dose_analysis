...
# g0 <- ggplot(data = filter(df_summarise, waning == "Default", vaccine_doses == "3 doses"), aes(x = as.Date(date), y  = vaccines_t/target_pop, linetype = rollout_rate)) +
#   geom_line(size = 1) +
...
g1b <- ggplot(data = filter(df_summarise, vacc_per_week == 0.02, waning == "Default", timestep < max(df_summarise$timestep)), aes(x = as.Date(date), y = inc_t/target_pop * 1e6, col = vaccine_doses)) +
  ...
  mutate(sensitivity_scenario = 
           if_else(waning == "Default" & rollout_rate == "Default", "Default",
                   if_else(waning == "Default" & rollout_rate == "Slower rollout", "Slower rollout",
                           if_else(waning == "Slower vacc waning" & rollout_rate == "Default", "Slower vacc waning", "None")))) %>%
...
g2b <- ggplot(data = df2, aes(x = sensitivity_scenario, y = inc_med/target_pop * 1e6, fill = vaccine_doses)) +
  geom_bar(position = "dodge", stat = "identity", alpha = 0.8) +
