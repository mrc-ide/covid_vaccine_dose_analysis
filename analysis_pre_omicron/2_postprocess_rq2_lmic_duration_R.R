...
scenarios <- read_csv("scenarios_rq2_lmic_dur_R.csv", show_col_types = FALSE)
df <- list.files(path = "raw_outputs/output_rq2_lmic_dur_R/", pattern = ".rds")
df <- map(paste0("raw_outputs/output_rq2_lmic_dur_R/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/df_summarise_rq2_lmic_dur_R.rds")
saveRDS(df_summarise_totals, "processed_outputs/df_summarise_totals_rq2_lmic_dur_R.rds")
