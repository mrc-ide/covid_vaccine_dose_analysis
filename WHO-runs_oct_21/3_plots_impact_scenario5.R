df_summarise <- readRDS("processed_outputs/df_summarise_scenario5.rds") %>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC"))) %>%
  filter(!(strategy_name != "Counterfactual" & timestep < 300))

df_summarise_totals <- readRDS("processed_outputs/df_summarise_totals_scenario5.rds") %>%
  mutate(income_group = factor(income_group, levels = c("LMIC", "LIC")))
...
p1
ggsave("plots/scenario5_impact_time.png", height = 4.5, width = 13)
...
p2  
ggsave("plots/scenario5_summary.png", height = 4.5, width = 8)
...
ggsave("plots/scenario5_summary_perdose.png", height = 4.5, width = 8)
