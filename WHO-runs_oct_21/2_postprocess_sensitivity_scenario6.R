...
scenarios <- read_csv("scenarios_scenario6_sensitivity.csv", show_col_types = FALSE)
df <- list.files(path = "output_cluster_scenario6_sensitivity/", pattern = ".rds")
df <- map(paste0("output_cluster_scenario6_sensitivity/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/df_summarise_scenario6_sensitivity.rds")
saveRDS(df_summarise_totals, "processed_outputs/df_summarise_totals_scenario6_sensitivity.rds")
saveRDS(filter(df_summarise, max_coverage == 0), "processed_outputs/df_counter_scenario6_sensitivity.rds")
