...
scenarios <- read_csv("scenarios_scenario5.csv", show_col_types = FALSE)
df <- list.files(path = "output_cluster_scenario5/", pattern = ".rds")
df <- map(paste0("output_cluster_scenario5/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/df_summarise_scenario5.rds")
saveRDS(df_summarise_totals, "processed_outputs/df_summarise_totals_scenario5.rds")
saveRDS(filter(df_summarise, max_coverage == 0), "processed_outputs/df_counter_scenario5.rds")
...
ggsave("plots/scenario5_setup.png", height = 12, width = 8)
