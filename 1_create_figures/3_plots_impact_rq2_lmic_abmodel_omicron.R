...
g1b <- ggplot(data = filter(df_summarise, vacc_per_week == 0.02, timestep < max(df_summarise$timestep), max_Rt == 5), aes(x = as.Date(date), y = inc_t/target_pop * 1e6, col = vaccine_doses)) +
