...
df <- left_join(df, scenarios, by = "scenario") %>%
  mutate(vaccine_doses = factor(vaccine_doses, levels = c(2,3), labels = c("2 doses", "2 doses + booster"))) %>%
...
df <- df %>%
  group_by(income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_label, vaccine_doses, vacc_start, variant_fold_reduction, dose_3_fold_increase, ab_model_infection, rollout_rate, vacc_per_week, max_Rt, t_d3, vfr) %>%
...
df_summarise <- df %>%
  ...
  group_by(timestep, income_group, target_pop, max_coverage, age_groups_covered,age_groups_covered_label, vaccine_doses, vacc_start, variant_fold_reduction, dose_3_fold_increase, ab_model_infection, max_Rt, vfr, rollout_rate, t_d3, vacc_per_week, deaths_med, deaths_lower, deaths_upper, inc_med, inc_lower, inc_upper, prop_R_med, total_doses_med) %>%
