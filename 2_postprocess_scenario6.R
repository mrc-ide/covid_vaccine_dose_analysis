...
scenarios <- read_csv("scenarios_scenario6.csv", show_col_types = FALSE)
df <- list.files(path = "output_cluster_scenario6/", pattern = ".rds")
df <- map(paste0("output_cluster_scenario6/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/df_summarise_scenario6.rds")
saveRDS(df_summarise_totals, "processed_outputs/df_summarise_totals_scenario6.rds")
saveRDS(filter(df_summarise, max_coverage == 0), "processed_outputs/df_counter_scenario6.rds")
...
g2
ggsave("plots/scenario6_cumulative_doses.png", height = 12, width = 8)
...
ggsave("plots/scenario6_setup.png", height = 12, width = 8)
