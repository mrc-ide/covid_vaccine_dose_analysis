...
scenarios <- read_csv("antibody_infection_model/scenarios_rq2_lmic_abmodel_1.csv", show_col_types = FALSE)
df <- list.files(path = "raw_outputs/output_rq2_lmic_abmodel_1/", pattern = ".rds")
df <- map(paste0("raw_outputs/output_rq2_lmic_abmodel_1/", df), readRDS)
df <- do.call(rbind,df)
...
saveRDS(df_summarise, "processed_outputs/ab_infection_model/df_summarise_rq2_lmic_abmodel_1.rds")
saveRDS(df_summarise_totals, "processed_outputs/ab_infection_model/df_summarise_totals_rq2_lmic_abmodel_1.rds")
