...
run_scenario_zerocovid <- 
  function(scenario = 1,
    ...
    R0_t5 <- as.Date(x = vfr_time1, format = "%m/%d/%Y")
    R0_t6 <- as.Date(x = vfr_time2, format = "%m/%d/%Y")
    ...
    dates <- c(R0_t0, R0_t1, R0_t2, R0_t3, R0_t4, R0_t5, R0_t6)
    dates <- dates[order(dates)]
    if (R0_t5 < R0_t3) {
      rt <-    c(1.1,   1.1,   0.2,   0.2,   (max_Rt_omicron - max_Rt), (max_Rt_omicron - max_Rt), max_Rt_omicron)
    } else {
