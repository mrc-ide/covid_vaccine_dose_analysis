...
run_scenario <- 
  function(scenario = 1,
    ...
    R0_t2 <- as.Date(x = "4/1/2020", format = "%m/%d/%Y")
    ...
    rt <-    c(2.0,   2.0,   0.9,   0.9,   1.2,   1.2,   1.0,   1.0,   3)
    ...
    vaccine_out <- get_vaccine_strategy(name, days_to_vacc_start = days_to_vacc_start, doses_per_day = doses_per_day, time_period = time_period, max_coverage = max_coverage, age_groups_covered = age_groups_covered, age_groups_covered_d3 = age_groups_covered_d3, vaccine_doses = vaccine_doses)
    ...
    saf_reps_summarise <- left_join(saf_reps_summarise, prop_R, by = "scenario1")
