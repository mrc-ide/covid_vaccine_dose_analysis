...
df1 <- df_summarise %>%
  filter(age_groups_covered == 15,
         max_coverage == 0.9)

# plot outputs: deaths
p_deaths <- ggplot(data = filter(df1,
  ...
  facet_wrap(dt~ab_model_infection, nrow =5, labeller = label_both, scales = "free") +
  geom_line() +
  theme_bw() +
...
p_deaths
ggsave("p_deaths_v3.png", p_deaths, height = 12, width = 12)


# plot outputs: infections
p_infections <- ggplot(data = filter(df1,
                                     strategy_name %in% c("Pre-vaccine introduction", "10y+ 2 doses, no booster", "10y+ 2 doses, booster 60y+", "10y+ 2 doses, booster 10y+"), max_Rt == 3, timestep < max(df_summarise$timestep))
...
p_infections
ggsave("p_infections.png", p_infections, height = 12, width = 12)
